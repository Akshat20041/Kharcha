...
Render may sleep and cold-start. Account loading retries network errors, timeouts and server failures automatically: up to four 20-second attempts, with 1/2/4-second delays (about 87 seconds maximum). The page explains the wait, cancels requests when the account/page changes, and retries when the browser reconnects. Persistent failures offer Retry or Sign out. A saved session normally opens without asking for a password again. Expense writes are never automatically retried, to prevent duplicates. Supabase has usage/storage limits and may pause inactive free projects. Free plans provide no production SLA, and quotas can change. Check [Render free limits](https://render.com/docs/free) and [Supabase pricing](https://supabase.com/pricing) before launch. Upgrade when cold starts, sustained traffic, storage, email delivery or backup requirements exceed a hobby beta. Do not add artificial keep-alive jobs. Stage 6 deployment automation and backup operations remain outside this stage.
